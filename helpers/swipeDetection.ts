...
export default class SwipeDetection {
	...
	boundStartTrackSwipe(event) {
		return;
	}

	boundTrackSwipe(event) {
		return;
	}

	boundStopTrackSwipe(event) {
		return;
	}
	...
	startTrackSwipe(startEvent) {
		...
		const pointer =
			startEvent.type.indexOf("touch") >= 0
		...
		document.body.addEventListener("mouseup", this.boundStopTrackSwipe, {
			passive: true,
		});
	...
	trackSwipe(swipeEvent) {
		const pointer =
			swipeEvent.type.indexOf("touch") >= 0
	...
	stopTrackSwipe(stopEvent) {
		if (
			...
			(stopEvent.type === "mouseout" &&
				!document.contains(stopEvent.relatedTarget))
		) {
			this.removeStopListener();
