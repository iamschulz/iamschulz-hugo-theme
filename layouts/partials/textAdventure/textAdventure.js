...
export default class TextAdventure extends Component {
	...
	setupDirections() {
		...
		Object.keys(DIRECTIONS.default).forEach((direction) => {
			this.directions[direction] = new Direction(
				DIRECTIONS.default[direction]
			);
		});
	}
	...
	movePC(inputDirection) {
		...
		const direction = Object.keys(this.directions).filter(
			(direction) =>
				direction.toLowerCase() === inputDirection.toLowerCase()
		)[0];
		...
		this.log(
			`💨 ${room.directions[direction].transition} ${this.rooms[newRoom].description}`
		);
	...
	inspect(thing) {
		if (!thing) {
			this.log(
				`👁️ You take a look around the ${this.getCurrentRoom().name}.`
			);
			this.inspect(this.getCurrentRoom().name);
			return;
		}

		const room = Object.keys(this.rooms).filter(
			(room) => room.toLowerCase() === thing.toLowerCase()
		)[0];
		const item = Object.keys(this.items).filter(
			(item) => item.toLowerCase() === thing.toLowerCase()
		)[0];
	...
	use(thing) {
		if (!thing) {
			console.log(
				'💡 What do you want to use? Try `ta.use("%cThing%c")`!',
				this.formats.items,
				this.formats.default
			);
			return;
		...
		if (item.interaction || (item.method && this[item.method])) {
			...
			console.log(
				`❌ You can't use %c${item.name}%c like that.`,
				this.formats.items,
				this.formats.default
			);
		}
	...
	resumeGame() {
		...
		this.log(
			store.lastTransition
				? `💨 ${store.lastTransition}`
				: `💨 You find yourself in the mansions ${
	...
	isItemInRoom(item) {
		return (
			Object.keys(this.getCurrentRoom().objects).filter(
				(itemName) => itemName.toLowerCase() === item.toLowerCase()
			).length > 0
		);
	}
